            ...
            repository.allFloors.take(1).collect { floors ->
                if (floors.isEmpty()) {
                    ...
                    repository.allFlats.take(1).collect { flats ->
                        val autoFlats = flats.filter { it.name.toIntOrNull() != null && it.sqFt == 1000.0 }
                        autoFlats.forEach { repository.deleteFlat(it) }
                        
                        val remainingFlats = flats.size - autoFlats.size
                        if (remainingFlats < 150) {
                            val floorId = floors.first().id
                            val needed = 150 - remainingFlats
                            for (i in 1..needed) {
                                repository.insertFlat(Flat(name = "", floorId = floorId, sqFt = 0.0))
                            }
                        }
